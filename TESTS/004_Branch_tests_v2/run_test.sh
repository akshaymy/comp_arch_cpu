...
if [ $# -ne 1 ]; then
	echo "ERROR: Please enter the Processor Name"
	exit 1
fi
...
iverilog tb.v ../../RTL/SoC.v ../../RTL/MEM/* ../../RTL/CORES/"${1}"/* -o test_bin
vvp test_bin

# Check correctness
diff -q rf_expected.dump rf_values.dump >/dev/null 2>&1
...
diff -q dmem_expected.dump dmem_values.dump >/dev/null 2>&1
